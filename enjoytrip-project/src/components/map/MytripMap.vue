<template>
    <div>
        ...
        <div class="input-group mb-3">
            ...
            <button @click="search" type="button" class="input-group-text btn-success"><i
                    class="fas fa-search"></i>
                Search</button>
        </div>
        <button @click="setBound">마커 한눈에 보기</button>
        <button @click="share">친구 추가하기</button>
        <div class="container">
...
function loadMaker() {
    ...
    for (var i = 0; i < mStore.mytripStore.len; i++) {
        ...
        let marker = new window.kakao.maps.Marker({
            ...
            position: new kakao.maps.LatLng(mStore.mytripStore.list[i].latitude, mStore.mytripStore.list[i].longitude)
        })
        marker.setMap(map.value)
        // LatLngBounds 객체에 좌표를 추가합니다
        bounds.extend(new kakao.maps.LatLng(mStore.mytripStore.list[i].latitude, mStore.mytripStore.list[i].longitude));
    }
    setBound()
}
