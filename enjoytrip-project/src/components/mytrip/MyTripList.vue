<template>
  <div>
    <div
      v-for="(mytrip, index) in store.mytripStore.mytripList"
      :key="index"
      class="mytrip-container mt-3"
    >
      <div class="mytrip-item">
        <div class="mytrip-title-container">
          ...
          <i
            v-if="mytrip.isOwner === 1"
            @click="deleteMytrip(mytrip.mytripId)"
            class="fas fa-trash mytrip-trash-icon"
          ></i>
        </div>
        <div class="user">
